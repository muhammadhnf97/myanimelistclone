...
const getApi = async(page) => {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/top/manga?page=${page || 1}`)
    return response
}
...
const Page = () => {
    ...
    const handlePagination = (page) => {
        setTopAnime([])
        router.push(`/topmanga?page=${page}`)
    }
  ...
  return (
    <>
    {
        ...
        <div className='max-w-7xl mx-auto bg-white p-3 space-y-5'>
            <h2 className='text-4xl font-bold text-primary text-center'>Top Manga</h2>
