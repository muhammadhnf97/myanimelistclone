...
const getApi = async(page) => {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/seasons/upcoming?page=${page || 1}`)
    return response
}
...
const Page = () => {
    ...
    useEffect(()=>{
        ...
        .then(data=>{
            ...
            setUpcoming(data.data.data.map(values=>{
            return {
  ...
  return (
    <>
    {
        ...
        <div className='max-w-7xl mx-auto bg-white p-3 space-y-5'>
            <h2 className='text-4xl font-bold text-primary text-center'>Upcoming Anime</h2>
            ...
            {upcoming.length > 0 && renderPaginationButtons()}
        </div>
    }
    </>
  )
